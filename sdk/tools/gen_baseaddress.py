...
def run_dir(target):
    ...
    for root, _, files in os.walk(target):
        for dll in [filename for filename in files if filename.endswith(ALL_EXTENSIONS)]:
            if dll not in EXCLUDE and not dll.startswith('api-ms-win-'):
                layout.add(os.path.join(root, dll), dll)
    ntdll_path = layout.found['ntdll.dll'].filename
    if target_file := get_target_file(ntdll_path):
    ...
    with output_file:
        output_file.write(f'# Generated from {target}\n')
